---
{"dg-publish":true,"permalink":"/工作/经纬度解析/","noteIcon":"","updated":"2023-12-16T17:05:55.112+08:00"}
---
...
      <canvas id="point-myCanvas"  style="width: 100%;height:auto;aspect-ratio:1;"></canvas>
...
      ctx.fillRect(18, canvas.height - 22, 4, 4); // 绘制一个小方块来标识原点
...
      ctx.moveTo(coordinates[0][0], canvas.height - coordinates[0][1]);
...
        ctx.arc(x, canvas.height - y, pointSize, 0, 2 * Math.PI);
...
        ctx.fillText(i, x, canvas.height - y + 4); // 将索引数字显示在点的内部
...
        infoBox.innerHTML += `Point ${i + 1}: (${x}, ${canvas.height - y})<br>`;
...
        const pointY = canvas.height - coordinates[i][1];
